...
public:
	HSPVRRAL(TProject & project,unsigned numberOfIterations=10) :
		m_Project(project)
	{
		m_NumberOfLevels=1;
	...
	virtual void DoRegistration(){
		for (int t = 0; t < m_Project.GetNumberOfFrames(); t++) {
			...
			for (unsigned int k = 1; k <= m_W; k++) {
				...
				int numberOfIterations { m_FirstRegistration ? 5 : 2 };
...
int main(int argc,char **argv){
	...
	if (argc != 6) {
		std::cerr << "Usage: " << argv[0] << " <directory> <prefix>  <first> <last> <iterations>" << std::endl;
		exit(-1);
	}
	...
	unsigned int MaxIterations = atoi(argv[5]);
